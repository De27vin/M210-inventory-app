...
$serverName = getenv('LDAP_NAME') ?: 'ldap'; // Default value if env variable is not set
...
$serverHost = getenv('LDAP_HOST') ?: 'ldap';
$servers->setValue('server', 'host', $serverHost);

/* The port your LDAP server listens on (no quotes). 1389 is standard. */
$serverPort = getenv('LDAP_PORT') ?: '1389';
$servers->setValue('server','port',$serverPort);

/* Array of base DNs of your LDAP server. Leave this blank to have phpLDAPadmin
   auto-detect it for you. */
$serverBase = getenv('LDAP_BASE') ?: 'dc=test,dc=ch';
$servers->setValue('server', 'base', array($serverBase));
...
/* The DN of the user for phpLDAPadmin to bind with. For anonymous binds or
   ...
   binds. */
$bindId = getenv('LDAP_BIND_ID') ?: 'cn=admin,dc=test,dc=ch';
$servers->setValue('login', 'bind_id', $bindId);
#  $servers->setValue('login','bind_id','cn=Manager,dc=test,dc=ch');
